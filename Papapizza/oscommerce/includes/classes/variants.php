...
*/
  ...
  class osC_Variants {
    ...
    static public function defineJavascript($products) {
      global $osC_Currencies, $osC_Specials;

      $string = '<script language="javascript" type="text/javascript">var combos = new Array();' . "\n";

      foreach ( $products as $product_id => $product )
	  {
		$price = addslashes($osC_Currencies->displayPrice($product['data']['price'], $product['data']['tax_class_id'])); 
		
		if($promo = $osC_Specials->GetRelativePrice($product_id))
		{
			$promo = $product['data']['price'] - ((!$promo['percentage']) ? $promo['value'] : ($product['data']['price'] * $promo['value'] / 100));
			$price = '<s>' . addslashes($osC_Currencies->displayPrice($product['data']['price'], $product['data']['tax_class_id'])) . '</s> <span class=\"productSpecialPrice\">' . $osC_Currencies->displayPrice($promo, $product['data']['tax_class_id']) . '</span>';
		}
		
        $string .= 'combos[' . $product_id . '] = new Array();' . "\n" .
                   'combos[' . $product_id . '] = { price: "' . $price . '", model: "' . addslashes($product['data']['model']) . '", availability_shipping: ' . (int)$product['data']['availability_shipping'] . ', values: [] };' . "\n";
