 ...
 *
 **/
class Layout extends CSScaffold_Plugins
{
 ...
 /**
	...
	{		
		...
		if( $settings = CSS::find_at_group('grid') )
		{
			...
			if(!is_readable(CSSCACHEPATH.'Layout'))
				mkdir(CSSCACHEPATH.'Layout');

			# Path to the image
			$img = CSSCACHEPATH . "Layout/{$lgw}_{$cw}_{$rgw}_{$bl}_grid.png";
