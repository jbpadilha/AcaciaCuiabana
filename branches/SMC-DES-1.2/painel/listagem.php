...
<div id="listagem">
    ...
    if(!is_null($collVoPessoa)) {
        ...
        foreach ($collVoPessoa as $pessoas) {
            ...
            $pessoaAtual = $pessoas;
            $txt .= '<label title="'.$formataData->toViewDate($pessoaAtual->getDataNascimentoPessoa()).'">Aniversário de '.$pessoaAtual->getNomePessoa().'</label>';
            $cont++;
        }
		echo '<p class="evento">Aniversariantes do dia ('.$cont.')</p>';
...
//Lista Vencimento de Garantia
    ...
    <p class="evento">Revisões agendadas</p>
    <?php
//Lista de Revisões
    ...
    if(!is_null($collVoRevisoes)) {
        ...
        foreach ($collVoRevisoes as $revisoes) {
            ...
            echo '<label title="">'.$veiculoAtual->getPlacaVeiculos().' - Revisão agendada</label>';
            $cont++;
        }
        echo '';
    }
