...
class ControlaEnquete extends ControlGeral {
	...
	private function preencheObjeto(Enquetes $enquete, $POST)
	{
		$enquete->_setFrom($POST);
		if($enquete->getvotos1Enquete() == '')
			$enquete->setvotos1Enquete(0);
		if($enquete->getvotos2Enquete() == '')
			$enquete->setvotos2Enquete(0);
		if($enquete->getvotos3Enquete() == '')
			$enquete->setvotos3Enquete(0);
		if($enquete->getvotos3Enquete() == '')
			$enquete->setvotos3Enquete(0);
		if($enquete->getvotos4Enquete() == '')
			$enquete->setvotos4Enquete(0);
		if($enquete->getvotos5Enquete() == '')
			$enquete->setvotos5Enquete(0);
		$this->MENSAGEM_ERRO = array_merge($this->MENSAGEM_ERRO, $enquete->validate());
	}
	
	public function cadastrar(Enquetes $enquete)
	{
		try {
			$enquete->save();
		}
	...
	public function deletar(Enquetes $enquete)
	{
		try {
	...
	public function alterar(Enquetes $enquete)
	{
		try
		{
			$enquete->update();
