		...
		if(!ProjetoUtil::verificaBrancoNulo($user) && !ProjetoUtil::verificaBrancoNulo($senha))
		{
			try {				
				...
				if($usuarios->find(true) > 0)
				{
					...
					header("Location:".ControlGeral::$PAGINA_INICIO_ADM_LOGADO);
				}
				else
				{
					$this->MENSAGEM_ERRO[] = Mensagens::getMensagem("USUARIO_SENHA_INCORRETO");
					header("Location:".ControlGeral::$PAGINA_INICIO_ADM."?mensagemErro=".urlencode(serialize($this->MENSAGEM_ERRO)));
				}
			}
			catch (Exception $e)
			{
