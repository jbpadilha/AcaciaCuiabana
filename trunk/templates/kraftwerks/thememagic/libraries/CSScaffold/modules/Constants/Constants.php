 ...
 *
 **/    
class Constants extends CSScaffold_Plugins  
{
	/**
	...
	{				
		...
		$constantsArray = CSScaffold::config('core.constants');  
		...
		{   
			if(is_array($value))
			{   
				foreach($constantsArray[$key] as $name => $val)
				{
				  	self::set($name, $val); 
				}   
			   unset($constantsArray[$key]);
			}
