...
function com_install()
{
	...
	if(!file_exists($mosConfig_absolute_path . "/images/com_adsmanager/fields/")){
		@mkdir($mosConfig_absolute_path . "/images/com_adsmanager/fields/");
	};
	...
	$database->setQuery("SELECT nbcats FROM #__adsmanager_config WHERE 1");
    $database->loadObjectList();
	if ($database->getErrorNum()) {
		$database->setQuery("ALTER TABLE #__adsmanager_config ADD `nbcats` int(4) NOT NULL DEFAULT '1'");
		$result = $database->query();
    }
	
	$database->setQuery("SELECT cbfieldvalues FROM #__adsmanager_fields WHERE 1");
    $database->loadObjectList();
    if ($database->getErrorNum()) {
		$database->setQuery("ALTER TABLE #__adsmanager_fields ADD `cbfieldvalues` INT(11) NOT NULL default '-1' AFTER `cb_field`");
		$result = $database->query();
    }
	...
	$database->setQuery("SELECT show_new FROM #__adsmanager_config WHERE 1");
    $database->loadObjectList();
	if ($database->getErrorNum()) {
		$database->setQuery("ALTER TABLE #__adsmanager_config ADD `show_new` tinyint(1) NOT NULL DEFAULT '1'");
		$result = $database->query();
		$database->setQuery("ALTER TABLE #__adsmanager_config ADD `nbdays_new` int(10) NOT NULL DEFAULT '5'");
		$result = $database->query();
		$database->setQuery("ALTER TABLE #__adsmanager_config ADD `show_hot` tinyint(1) NOT NULL DEFAULT '1'");
		$result = $database->query();
		$database->setQuery("ALTER TABLE #__adsmanager_config ADD `nbhits` int(10) NOT NULL DEFAULT '100'");
		$result = $database->query();
    }
		
	$database->setQuery("SELECT count(*) FROM #__adsmanager_adcat WHERE 1");
	$total = $database->loadResult();
	if ($total == 0)
	{
		$database->setQuery("SELECT id,category FROM #__adsmanager_ads WHERE 1");
		$listads = $database->loadObjectList();
		$query = "INSERT INTO #__adsmanager_adcat (`adid`,`catid`) VALUES ";
		foreach($listads as $key => $ad)
		{
			if ($key != 0)
				$query .= ",";
			$query .= "('$ad->id','$ad->category')";
		}
	 ...
	 //Update to 2.5 RC 2
    $database->setQuery("SELECT link_text FROM #__adsmanager_config WHERE 1");
    $database->loadObjectList();
	if ($database->getErrorNum()) {	
		$database->setQuery("ALTER TABLE #__adsmanager_fields ADD `link_text` varchar( 255 ) NOT NULL DEFAULT ''");
		$result = $database->query();
		
		$database->setQuery("ALTER TABLE #__adsmanager_fields ADD `link_image` varchar( 255 ) NOT NULL DEFAULT ''");
		$result = $database->query();
    }
    
    $database->setQuery("INSERT IGNORE INTO `#__adsmanager_config` VALUES (1, '1.0.1', 20, 2048000, 400, 300, 150, 100, 1,2,1,1,1,1, 'joomprod.com', '<p align=\"center\"><strong>Welcome to Ads Section.</strong></p><p align=\"left\">The better place to sell or buy</p>',0,0,'Inform the users about the rules here...',2,1,0,1,30,1,7,'Add This Text to recall message<br />','default',150,150,30,30,0,-1,0,0,0,1,1,5,1,100)");
    $result = $database->query();
	
	$database->setQuery("SELECT * FROM `#__adsmanager_config` WHERE 1 LIMIT 1");
    $database->loadObject($conf);
	
	
	$database->setQuery("ALTER TABLE #__adsmanager_field_values CHANGE `fieldvalue` `fieldvalue` VARCHAR( 50 ) NOT NULL DEFAULT '0'");
    $result = $database->query();
	
	//Need to be after adsmanager_config INSERT
	$database->setQuery("SELECT expiration_date FROM #__adsmanager_ads WHERE 1");
    $database->loadObjectList();
    if ($database->getErrorNum()) {
		$database->setQuery("ALTER TABLE #__adsmanager_ads ADD `expiration_date` DATE NOT NULL AFTER `date_recall`");
		$result = $database->query();
		
		$database->setQuery("UPDATE #_adsmanager_ads SET expiration_date = DATE_ADD(date_created, INTERVAL ".$conf->ad_duration." DAY) WHERE 1");
		$database->query();
    }
?>
