    ...
    function initialize() {
      ...
      if ($osC_Services->isStarted('specials')) {
        if ((BOX_SPECIALS_CACHE > 0) && $osC_Cache->read('box-specials-' . $osC_Language->getCode() . '-' . $osC_Currencies->getCode(), BOX_SPECIALS_CACHE)) {
          ...
          $Qspecials = $osC_Database->query('select p.products_id, p.has_children, p.products_price, p.products_tax_class_id, pd.products_name, pd.products_keyword, s.specials_new_products_price, s.value, s.percentage, i.image from :table_products p left join :table_products_images i on (p.products_id = i.products_id and i.default_flag = :default_flag), :table_products_description pd, :table_specials s where s.status = 1 and s.products_id = p.products_id and p.products_status = 1 and p.products_id = pd.products_id and pd.language_id = :language_id order by s.specials_date_added desc limit :max_random_select_specials');
          ...
          if ($Qspecials->numberOfRows()) {
			...
			{
				$prod = new osC_Product($Qspecials->valueInt('products_id'));
				$data['products_price'] = $prod->getPriceFormated(true);
