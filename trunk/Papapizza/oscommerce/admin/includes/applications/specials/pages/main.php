...
<?php
  $Qspecials = $osC_Database->query('select p.products_id, pd.products_name, p.products_price, p.has_children, s.specials_id, s.specials_new_products_price, s.specials_date_added, s.specials_last_modified, s.expires_date, s.date_status_change, s.status, s.value, s.percentage from :table_products p, :table_specials s, :table_products_description pd where p.products_id = pd.products_id and pd.language_id = :language_id and p.products_id = s.products_id order by pd.products_name');
...
?>
    ...
    <tr onmouseover="rowOverEffect(this);" onmouseout="rowOutEffect(this);" <?php echo (($Qspecials->valueInt('status') !== 1) ? 'class="deactivatedRow"' : '') ?>>
      ...
      <td>
	<?php
	if(!$Qspecials->valueInt('has_children'))
		echo '<span class="oldPrice">' . $osC_Currencies->format($Qspecials->value('products_price')) . '</span><span class="specialPrice">' . $osC_Currencies->format($Qspecials->value('specials_new_products_price')) . '</span>';
	else
	{
		echo '<span class="specialPrice">* '; 

		if(($Qspecials->valueInt('percentage')))
			echo $Qspecials->valueInt('value') . '%';
		else
			echo $osC_Currencies->format($Qspecials->value('value'));
