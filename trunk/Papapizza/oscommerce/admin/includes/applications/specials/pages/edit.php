...
*/
  ...
  $Qspecial = $osC_Database->query('select p.products_id, p.has_children, pd.products_name, p.products_price, p.products_tax_class_id, s.specials_new_products_price, s.expires_date, s.start_date, s.status, s.value, s.percentage from :table_specials s, :table_products p, :table_products_description pd where s.specials_id = :specials_id and s.products_id = p.products_id and p.products_id = pd.products_id and pd.language_id = :language_id');
...
$price = (!$Qspecial->valueInt('has_children')) ? $Qspecial->valueDecimal('specials_new_products_price') : $Qspecial->valueDecimal('value');
if($Qspecial->valueInt('percentage'))
	$price = (int) $price . '%';
?>
...
<div class="infoBoxContent">
  ...
  <p><?php echo '<b>' . $Qspecial->value('products_name') . ((!$Qspecial->valueInt('has_children')) ? ' (' . $osC_Currencies->format($Qspecial->valueDecimal('products_price')) . ')' : '') . '</b>' . osc_draw_hidden_field('products_id', $Qspecial->valueInt('products_id')); ?></p>
  <p><?php echo '<b>' . $osC_Language->get('field_price_net') . '</b><br />' . osc_draw_input_field('specials_price', $price, 'onkeyup="updateGross(\'specials_price\', event)"'); ?></p>
  <p><?php echo '<b>' . $osC_Language->get('field_price_gross') . '</b><br />' . osc_draw_input_field('specials_price_gross', $price, 'onkeyup="updateNet(\'specials_price\', event)"'); ?></p>
