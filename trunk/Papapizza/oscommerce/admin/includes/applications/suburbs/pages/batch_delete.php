...
<?php
  $Qadmins = $osC_Database->query('select suburbs_id, suburbs_name, suburbs_price from :table_suburbs where suburbs_id in (":id") order by suburbs_name');
  ...
  while ( $Qadmins->next() ) {
    $names_string .= osc_draw_hidden_field('batch[]', $Qadmins->valueInt('id')) . '<b>' . $Qadmins->valueProtected('suburbs_name') . '</b> ' . $Qadmins->valueProtected('suburbs_price') . '</b>, ';
  }
