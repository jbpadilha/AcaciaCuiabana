...
class JElementJxtcewven extends JElement {
	var	$_name = 'Jxtcewven';
	function fetchElement($jxtcname, $jxtcvalue, &$jxtcnode, $jxtccontrol_name)	{
		$jxtccat=array();$jxtcref=array();$jxtcoptions=array();
		$db	=& JFactory::getDBO();
		$q = "SELECT id, venue as name FROM #__eventlist_venues WHERE published=1 ORDER BY ordering";
		$db->setquery($q);
		$result=$db->loadObjectList();
		array_unshift($result,(object)array('id'=>0,'name'=>'ALL VENUES'));
		$size = count($result);
		$size = ceil($size/10);
		if ($size < 5) $size = 5;
		if ($size > 20) $size = 20;
