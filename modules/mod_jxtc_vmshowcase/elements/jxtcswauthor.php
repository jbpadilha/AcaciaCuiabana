...
class JElementJxtcswauthor extends JElement {
	var	$_name = 'Jxtcswauthor';
	function fetchElement($jxtcname, $jxtcvalue, &$jxtcnode, $jxtccontrol_name)	{
		$db	=& JFactory::getDBO();
		$q = "SELECT distinct(si.addedby) as id, u.name FROM #__seyret_items as si, #__users as u where u.id = si.addedby ORDER BY u.name";
		$db->setquery($q);
		$result=$db->loadObjectList();
		array_unshift($result,(object)array('id'=>0,'name'=>'ALL AUTHORS'));
		$size = count($result);
		$size = ceil($size/10);
		if ($size < 5) $size = 5;
		if ($size > 20) $size = 20;
