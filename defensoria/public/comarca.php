...
function cadastra()
{
	if ( $('#nome').val() == '' ) {
		...
		var formulario = $('#comarca').serialize(true);
		enviaFormulario($('#comarca').attr("action"),'page',formulario);
	}
}
</script>
<form name="comarca" id="comarca" method="post" action="../application/recebePostGet.php">
	<?php
	$comarca = null;
	$comarca = new Comarca();
	...
	<input type="hidden" id="funcao" name="funcao" value="<?=(isset($_GET['idComarca']))?"alterar":"cadastrar"?>"/>
	...
	<table>
		<tr>
			...
			<td width="49"><input type="button" name="submit" onclick="cadastra();" id="submit" value="<?=(isset($_GET['idComarca']))?"Alterar":"Cadastrar"?>"/></td>
		</tr>
	</table>
</form>
<?php
